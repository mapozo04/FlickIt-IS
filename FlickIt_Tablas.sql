...
CREATE TABLE Proveedores (
    ...
    CONSTRAINT FlickIt_CHK_TIPO_PROV CHECK (tipo_proveedor IN ('pequeño','grande'))
);

-- 3. Tabla Productos(id_producto, id_vendedor, nombre, descripcion, precio, stock, tipo_venta, fecha_publicacion)
...
CREATE TABLE Productos(
    ...
    -- estado = 'pendiente' -> producto recién subido
...
-- 4. TABLA MultimediaProducto(id_multimedia, id_producto, url_archivo, tipo_archivo)
...
-- 5. Interacciones/swipes(id_interaccion, id_usuario, id_producto, tipo_interaccion, fecha_interaccion)
CREATE TABLE Interacciones(
    ...
    CONSTRAINT FlickIt_CHK_TIPO_INTERACT CHECK (tipo_interaccion IN ('like','dislike'))
);

-- 6. TABLA Pedidos(id_pedido, id_cliente, fecha_pedido, estado_pedido, total_precio)
...
-- 6.1. TABLA DetallesPedido(id_detalle, id_pedido, id_producto, cantidad, precio_unitario)
...
-- 7. TABLA Descuentos(id_descuento, codigo_descuento,porcentaje, fecha_expiracion)
